...
const nwork = require('./streamer-network.js');
...
async function btAmpManagement(connect, bdaddr){
  ...
  if (connect === false) {//isFile is NOT NEEDED here  fix
...
function isFile(path) {
  ...
  try {
    existance =
        execSync(`sudo find ${path} | tr -s ' '  `,
            { uid: 1000, gid: 1000, encoding: 'utf8', timeout: 10000 });
      //console.log(aux.timeStamp(), "bt: No ERRORS yields:", existance, "[*]");
      }
  catch (err) {
    //console.log(aux.timeStamp(),"bt: is there an /etc/asound.conf file?", err);
    return false;
  };
  //if existance is not as same as path the file doesn't exists
  if (existance != path) {
      //console.log(aux.timeStamp(), "bt: no path to /etc/asound.conf file;", false, "[!]");
      //console.log(aux.timeStamp(), "bt: not equal to path, yields:", false, "[!]");
      result = false;     //CLI find found something else
    };
  return result;
};
